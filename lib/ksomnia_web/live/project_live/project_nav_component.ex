defmodule KsomniaWeb.AppLive.ProjectNavComponent do
  ...
  def render(assigns) do
    ...
    <nav class="px-7 py-4 isolate flex divide-x divide-gray-200 rounded-lg">
      ...
      <%= live_redirect [
        to: Routes.project_members_path(@socket, :members, @project.id),
        class: "text-slate-500 hover:text-slate-700 group relative min-w-0 flex-1 overflow-hidden bg-slate-50 py-4 px-4 text-sm font-medium text-center hover:bg-gray-50 focus:z-10"
      ] do %>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 inline-block">
      ...
      <%= live_redirect [
        ...
        <span>Settings</span>
        <span aria-hidden="true" class={"#{if false, do: "bg-indigo-500 absolute inset-x-0 bottom-0 h-0.5", else: "bg-transparent absolute inset-x-0 bottom-0 h-0.5"}"}></span>
      <% end %>
    </nav>
    """
  end
